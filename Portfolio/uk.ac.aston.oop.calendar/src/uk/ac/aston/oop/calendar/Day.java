...
public class Day {
	...
	public boolean makeAppointment(int time, Appointment appointment) {
		...
		//we check if the time is within the start of day
		if(validTime(time)) {
			
			//we want to see if there an entry at this position
			if(appointments[time - START_OF_DAY] == null) {
				//we want to check the duration,
				if(appointment.getDuration() > 1) {
					
					
					//we check here if the time will be valid or not if the duration is above one, incase it'll go above
					if(!validTime(time + appointment.getDuration())) {
						
						return false;
					}
					
					//we check if all slots are free that we want to use
					for(int i = 0; i <= (appointment.getDuration() - 1); i++) {
						System.out.println(i);
						if (appointments[(time + i) - START_OF_DAY] != null) {
							return false;
						}
					}
					//we want to put this appointment in each slot of the array index.
					for(int i = 0; i <= (appointment.getDuration() - 1); i++) {
						
						//We add i onto the time to fill each slot the appointment is booked
						
						appointments[(time + i) - START_OF_DAY] = appointment;
